...
def process_diagram(diagram_commands,ruta_dot, ruta_uc):
    comandos = []
    diagram_name = diagram_commands.pop(0)
    three_digit_key = diagram_commands.pop(0)
    
    filename = f"{ruta_dot}/FL_ADMIN_{three_digit_key}.dot"
    ...
    ruta_img = "{ruta_uc}/FL_ADMIN_" + three_digit_key + ".png"
...
def main(nombre_entidad, ruta_commands_dot, ruta_dot, ruta_uc):
    comandos = []

    with open(ruta_commands_dot, 'r') as f:
    ...
    for diagram_commands in diagrams:
        comandos.extend(process_diagram(diagram_commands, ruta_dot, ruta_uc))

    cmds_key = nombre_entidad + "_comandos_con_claves_dot.txt"
    with open(cmds_key, 'w') as f_out:
        for cmd_list in comandos:
            for line in cmd_list:
                f_out.write(line + '\n')
        
    print(f"Comandos para la entidad '{nombre_entidad}' registrados en comandos_con_claves_dot.txt.")

if __name__ == "__main__":
    if len(sys.argv) == 5:
        nombre_entidad = sys.argv[1]
        ruta_commands_dot = sys.argv[2]
        ruta_dot = sys.argv[3]
        ruta_uc = sys.argv[4]
        
        main(nombre_entidad, ruta_commands_dot, ruta_dot, ruta_uc)
    else:
        print("Uso: python s1_dot_gen.py <nombre_entidad> <ruta_comandos_dot> <ruta_dot> <ruta_uc>")
        sys.exit(1)